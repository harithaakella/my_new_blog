...
<html lang="en">
	...
	<head>
		...
		<title><?php echo $this->config->item('site_name')?> - Edit Post</title>
	...
	<body>
		...
		<?php $this->view('dashboard/_menu')?>
			...
			<div id="page-wrapper">
				<div class="row">
					<div class="col-lg-12">
						<h1 class="page-header">Edit - <?php echo $post->title ?></h1>
					</div>
					<!-- /.col-lg-12 -->
				</div>
				<!-- /.row -->
				...
				<div class="row">
					<div class="col-lg-12">
						...
						<?php echo form_open('dashboard/posts/'.$post->id.'/edit')?>
							<div class="form-group">
								<label>Post Title</label>
								<input class="form-control" name="title" value="<?php echo $post->title ?>" placeholder="E.g. My blog post no 1">
								...
								<div class="checkbox">
									<label>
										<input type="checkbox" name="category[]" value="<?php echo $cat->id?>" 
											<?php if( array_key_exists($cat->id, $post_categories) ){ echo 'checked';}?>><?php echo $cat->name?>
									</label>
								</div>
								<?php endforeach; else:?>
								<div class="checkbox">
